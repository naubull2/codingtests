...
class Solution:
    def verticalTraversal(self, root: Optional[TreeNode]) -> List[List[int]]:
        queue = deque([(0, root)]) # we put (col, node)
        columns = {} # collect by column -> later merge them in to an array
        row_span = 1
        row = 0
        row_cnt = 0
        ...
        while queue:
            ...
            if row_cnt == row_span: # on reaching the end of the row, start next row
                row_cnt = 0
                row +=1
                row_span = len(queue)
        ...
        for col in sorted(columns.keys()):
            ans.append([e[1] for e in sorted(columns[col], key=lambda x: x)])
        return ans
...
def test(tree, output):
    T = build_tree(tree, constructor=TreeNode)
    assert Solution().verticalTraversal(T) == output
